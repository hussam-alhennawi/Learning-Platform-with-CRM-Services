...
<div class="tab-pane fade active in" id="Events">
    <h3 class="title-section title-bar-high mb-40">Events Info</h3>
    <div class="orders-info">
        <canvas id="myChart" height="40vh" width="60vw"></canvas>
        <script>
            var ctx = document.getElementById('myChart');
            var labels = [ @foreach($events as $event) '{{$event->name}} ({{date("d M , Y",strtotime($event->started_at))}})', @endforeach]; 
                    ...
                    datasets: [
                        {
                            ...
                            data: [ @foreach($events as $event) '{{$event->GoingStudents()->count()}}', @endforeach],
                            backgroundColor: [
                                <?php 
                                    $color = ['r'=>0, 'g'=>0, 'b'=>0];
                                    $colors = [];
