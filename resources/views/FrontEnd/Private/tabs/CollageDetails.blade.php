<div class="tab-pane fade active in" id="CollageDetails">
    <h3 class="title-section title-bar-high mb-40"><a class="prev-page page-link"><i class="fa fa-level-up"></i>Back </a>| Number of students at each class by years</h3>
    <div class="orders-info">
        <canvas id="myChart" height="40vh" width="60vw"></canvas>
        <script>
            var ctx = document.getElementById('myChart');
            var labels = [ @foreach($subjects as $subject) '{{$subject->name_en}}', @endforeach]; 
            var myChart = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [
                        ...
                        {
                            label: 'Students At ({{$sy->study_year}})',
                            data: [ @foreach($subjects as $subject) 
                                        @if($subject->classesByYear($sy->study_year))
                                            {{$subject->classesByYear($sy->study_year)->StudentsRegistredAtClass()->count()}}
                                        @else
                                            null
                                        @endif
                                        ,
