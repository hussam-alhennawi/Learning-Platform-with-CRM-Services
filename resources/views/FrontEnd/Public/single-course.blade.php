                                ...
                                <div class="row">
                                    <div class="col-lg-12 col-md-12 col-sm-12">
                                        <ul class="course-details-tab-btn">
                                            ...
                                            <li><a href="#curriculum" data-toggle="tab" aria-expanded="false">Topics</a></li>
                                    ...
                                    <div class="col-lg-12 col-md-12 col-sm-12">
                                        <div class="tab-content">
                                            ...
                                            <div class="tab-pane fade" id="curriculum">
                                                <h3 class="sidebar-title">{{$course->category->name}} : {{$course->title}}</h3>
                                                <div class="panel-group curriculum-wrapper" id="accordion">
                                                    @foreach ($course->topics as $topic)
                                                        <div class="panel panel-default">
                                                            <div class="panel-heading">
                                                                <div class="panel-title">
                                                                    <a aria-expanded="false" class="accordion-toggle collapsed" data-toggle="collapse" data-parent="#accordion" href="#collapse{{$topic->id}}">
                                                                        <ul>
                                                                            <li><i class="fa fa-file-o" aria-hidden="true"></i></li>
                                                                            <li>{{$topic->name}}</li>
                                                                            <li>{{$topic->description}}</li>
                                                                            <li><i class="fa fa-clock-o" aria-hidden="true"><span> {{$topic->contents()->count()}} Contents</span></i></li>
                                                                        </ul>
                                                                    </a>
                                                                </div>
                                                            </div>
                                                            <div aria-expanded="false" id="collapse{{$topic->id}}" role="tabpanel" class="panel-collapse collapse">
                                                    ...
                                                    <ul>
                                                        <li>
                                                            <div class="skilled-lecturers-img">
                                                                <a href="{{route('user',$course->lecturer->id)}}"><img src="{{url('photos/profiles')}}/{{$course->lecturer->image}}" class="img-responsive" alt="{{$course->lecturer->full_name()}}"></a>
                                                            </div>
                                                            <div class="skilled-lecturers-content">
                                                                <h4><a href="{{route('user',$course->lecturer->id)}}">{{$course->lecturer->full_name()}}</a></h4>
                                                                <p>Lecturer At 
                                                            ...
                                                            <div class="skilled-lecturers-schedule">
                                                                <ul>
                                                                    <li>
                                                                        <h4>Teach</h4>
                                                                        <p>{{$course->lecturer->courses()->count()}} Courses</p>
                                                                    </li>
                                                                    <li>
                                                                        <h4>Contact</h4>
                                    ...
                                    <div class="courses-box1">
                                        <div class="single-item-wrapper">
                                            <div class="courses-img-wrapper hvr-bounce-to-right">
                                                <img class="img-responsive" src="{{url('/photos/courses')}}/{{$rCourse->image}}" alt="courses">
                                                <a href="{{url('/single-course')}}/{{$rCourse->id}}"><i class="fa fa-link" aria-hidden="true"></i></a>
                                            </div>
                                            <div class="courses-content-wrapper">
                                                <h3 class="item-title"><a href="{{url('/single-course')}}/{{$rCourse->id}}">{{$rCourse->title}}</a></h3>
                                        ...
                                        @role(['Internal_student','External_student'])
                                            @if($course->accept)
                                                ...
                                                @elseif($course->register->status == "Active")
                                                    <p>You're Active in this course at <a href="{{url('/content')}}/{{$course->accept->progress}}">{{$course->accept->content->title}}</a></p>
                                                @endif
                                            ...
                                            @else
                                                <a class="enroll-btn" href="#cou" id="register-button" onclick="openForm()">Register In THis Course</a>
                                                <div class="register-form" id="register-form" style="display: none;">
                                                    <div class="title-default-left-bold">Send Register Request</div>
                                                    <form method="POST" action="{{ route('student.reg') }}" enctype="multipart/form-data">
                                                    @csrf
                                                        @if(($course->cost != 0))
                                                            <label>Payment Check</label>
                                                            <input type="file" name="payment_check" class="form-control{{ $errors->has('payment_check') ? ' is-invalid' : '' }}" accept=".pdf" id="payment_check" required autofocus />
                                                        ...
                                                        <button class="default-big-btn" type="submit" style="width: auto;padding: 6px 20px;" value="Register">Register</button>
                                                        <style>
                                                            .form-cancel:hover{
                                                                cursor: pointer;
                                                            }
                                                        </style>
                                                        <a class="default-big-btn form-cancel" style="width: auto;padding: 6px 20px;" onclick="closeForm()">Cancel</a>
                                                    </form>
                                                </div>
                                            @endif
                                        @endrole
                                    ...
                                    <div class="sidebar-course-reviews">
                                        ...
                                        <div class="skill-area">
                                            <div class="progress star5">
                                                ...
                                                <div data-wow-delay="1.2s" data-wow-duration="1.5s" style="width: {{($course->rates[0] > 0)?($course->rates[5]/$course->rates[0])*100 : 0}}%; visibility: visible; animation-duration: 1.5s; animation-delay: 1.2s; animation-name: fadeInLeft;" data-progress="{{($course->rates[0] > 0)?($course->rates[5]/$course->rates[0])*100 : 0}}%" class="progress-bar wow fadeInLeft  animated"></div><span>{{$course->rates[5]}}</span>
                                            </div>
                                            <div class="progress star4">
                                                <div class="lead">4 Stars</div>
                                                <div data-wow-delay="1.2s" data-wow-duration="1.5s" style="width: {{($course->rates[0] > 0)?($course->rates[4]/$course->rates[0])*100 : 0}}%; visibility: visible; animation-duration: 1.5s; animation-delay: 1.2s; animation-name: fadeInLeft;" data-progress="{{($course->rates[0] > 0)?($course->rates[4]/$course->rates[0])*100 : 0}}%" class="progress-bar wow fadeInLeft  animated"></div><span>{{$course->rates[4]}}</span>
                                            </div>
                                            <div class="progress star3">
                                                <div class="lead">3 Stars</div>
                                                <div data-wow-delay="1.2s" data-wow-duration="1.5s" style="width: {{($course->rates[0] > 0)?($course->rates[3]/$course->rates[0])*100 : 0}}%; visibility: visible; animation-duration: 1.5s; animation-delay: 1.2s; animation-name: fadeInLeft;" data-progress="{{($course->rates[0] > 0)?($course->rates[3]/$course->rates[0])*100 : 0}}%" class="progress-bar wow fadeInLeft  animated"></div><span>{{$course->rates[3]}}</span>
                                            </div>
                                            <div class="progress star2">
                                                <div class="lead">2 Stars</div>
                                                <div data-wow-delay="1.2s" data-wow-duration="1.5s" style="width: {{($course->rates[0] > 0)?($course->rates[2]/$course->rates[0])*100 : 0}}%; visibility: visible; animation-duration: 1.5s; animation-delay: 1.2s; animation-name: fadeInLeft;" data-progress="{{($course->rates[0] > 0)?($course->rates[2]/$course->rates[0])*100 : 0}}%" class="progress-bar wow fadeInLeft  animated"></div><span>{{$course->rates[2]}}</span>
                                            </div>
                                            <div class="progress star1">
                                                <div class="lead">1 Stars</div>
                                                <div data-wow-delay="1.2s" data-wow-duration="1.5s" style="width: {{($course->rates[0] > 0)?($course->rates[1]/$course->rates[0])*100 : 0}}%; visibility: visible; animation-duration: 1.5s; animation-delay: 1.2s; animation-name: fadeInLeft;" data-progress="{{($course->rates[0] > 0)?($course->rates[1]/$course->rates[0])*100 : 0}}%" class="progress-bar wow fadeInLeft  animated"></div><span>{{$course->rates[1]}}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
        ...
        $('.rate-star').click(function(){
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    $('.my-rate').html(rate);
                    var newRate = (Math.round(response.data.rate * 100) / 100).toFixed(2);
