...
@section('content')
    <div id="breadcrumb"> <a href="{{route('management')}}" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="{{route('courses.index')}}">Courses</a> <a href="#" class="current">Edit Course</a> </div>
            ...
            <div class="span12">
                <div class="widget-box">
                    ...
                    <div class="widget-content nopadding">
                        <form class="form-horizontal" method="post" action="{{route('courses.update',$course->id)}}" name="basic_validate" id="basic_validate" novalidate="novalidate" enctype="multipart/form-data">
                                ...
                                <div class="controls">
                                    <select name="category_id" style="width: 415px;">
                                        @foreach ($categories as $category)
                                            <option value="{{$category->id}}" {{($course->category_id == $category->id)?'selected':''}}>{{$category->parentCategory->name}}: {{$category->name}}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="controls">
                                    <select name="lecturer_id" style="width: 415px;">
                                        @foreach ($lecturers as $lecturer)
                                            <option value="{{$lecturer->id}}" {{($course->lecturer_id == $lecturer->id)?'selected':''}}>{{$lecturer->first_name}} {{$lecturer->last_name}}</option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="control-group{{$errors->has('skills')?' has-error':''}}">
                            ...
                            <div class="controls">
                                <textarea name="skills" id="skills" required>{{$course->skills}}</textarea>
                        ...
                        <div class="control-group{{$errors->has('cost')?' has-error':''}}">
                            ...
                            <div class="controls">
                                <input type="number" min="0" step="0.01" name="cost" id="cost" value="{{$course->cost}}" required>
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                ...
                                @else
                                    ...
                                    &nbsp;&nbsp;&nbsp;
                                    <a href="javascript:" rel="{{$course->id}}" rel1="delete-cour-img" class="btn btn-danger btn-mini deleteRecord">Delete Old Image</a>
                                    <img src="{{url('/photos/courses')}}/{{$course->image}}" width="50" alt="">
                                @endif
                            </div>
                        </div>
                            <div class="control-group">
